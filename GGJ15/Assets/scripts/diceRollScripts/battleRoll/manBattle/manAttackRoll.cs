...
public class manAttackRoll : MonoBehaviour {
	...
	void Update () {
		...
		if (grounded && rigidbody.velocity.magnitude < manAttackRoll.deltaV) {
			{
	...
	IEnumerator DiceTimer(){
		...
		{
			Application.LoadLevel("manBattleDefence");
		...
		{
			Application.LoadLevel("manVictory");
