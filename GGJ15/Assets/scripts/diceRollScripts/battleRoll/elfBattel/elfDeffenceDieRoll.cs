...
public class elfDeffenceDieRoll : MonoBehaviour {
	...
	void Update () {
		...
		if (grounded && rigidbody.velocity.magnitude < elfDeffenceDieRoll.deltaV) {
			{
	...
	IEnumerator DiceTimer(){
		...
		if (die1Value.currentValue >= 3) {
			GameDataScript.health = GameDataScript.health - 4;
		}
		...
		if (GameDataScript.health >= 1)
		{
			...
			Application.LoadLevel ("elfBattleAttack");
		}
